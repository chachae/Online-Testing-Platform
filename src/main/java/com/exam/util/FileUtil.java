...
public class FileUtil extends cn.hutool.core.io.FileUtil {
  ...
  public static File upload(MultipartFile file, String filePath) {
    String time = DateUtil.getFormatLocalDateTimeStr("yyyyMMddhhmmssS");
    String name = getFileNameNoEx(file.getOriginalFilename());
    String suffix = getExtensionName(file.getOriginalFilename());
    String nowStr = StrUtil.DASHED + time;
    try {
      String fileName = name + nowStr + StrUtil.DOT + suffix;
      ...
      // 文件写入
      // 特别注意：当MultipartFile执行保存的时候（MultipartFile.transferTo(file)），会把临时文件删除
      // ，再执行MultipartFile.getSize()的时候，就会出现找不到临时文件的异常。
  ...
  public static void checkSize(long maxSize, long size) {
    ...
    int len = 1024 * 1024;
    if (size > (maxSize * len)) {
      throw new ServiceException("文件超出规定大小，最大支持 " + maxSize + " MB");
    }
  }
}
